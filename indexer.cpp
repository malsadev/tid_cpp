                                 ...
                                 std::unordered_map<std::string, std::unordered_set<CXCursorKind>>& token_map)
{
    ...
    for (auto token_type_pair : token_map)
    {
        ...
        {
            external_calls_map[std::string(entry_filename)].push_back(cursor_spelling);
        }
...
int index_method_decls(std::string_view file_path, std::unordered_map<std::string, std::vector<std::string>>& method_decls, CXIndex index)
{
                                  ...
                                  std::unordered_map<std::string, std::vector<std::string>>& files_map)   //no need to do reference binding for pointer CXIndex, copying pointers is cheap
{
    ...
    for (auto& header: files_map[std::string(file_path)])
    {
        ...
        if (index_method_decls(header, method_decls, index) == 1) //don't propagate files map just yet
        {
...
int indexer::index(std::string_view file_path)
{
    ...
    {
        std::unordered_map<std::string, std::vector<std::string>> tmp = indexer::external_calls;
    ...
    if(index_included_method_decls(file_path, indexer::m_index, indexer::method_decls, indexer::working_dir_repr) == 1)
    {
